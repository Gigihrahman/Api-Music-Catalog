...
func Test_service_GetRecommendation(t *testing.T) {
	...
	tests := []struct {
		name    string
		args    args
		want    *spotify.RecommendationsResponse
		wantErr bool
	...
	}{
		{
			...
			wantErr: false,
			mockFn: func(args args) {
				mockSpotifyOutbond.EXPECT().GetRecommendation(gomock.Any(), 10, "trackID").Return(&spotifyRepo.SpotifyRecommendationResponse{
				...
				mockTrackActivitiesRepo.EXPECT().GetBulkSpotifyIDs(gomock.Any(), uint(1), []string{"3z8h0TU7ReDPLIbEnYhWZb", "4u7EnebtmKWzUH433cf5Qv"}).Return(map[string]trackacktivities.TrackActivity{
			...
			mockFn: func(args args) {
				...
				mockTrackActivitiesRepo.EXPECT().GetBulkSpotifyIDs(gomock.Any(), uint(1), []string{"3z8h0TU7ReDPLIbEnYhWZb", "4u7EnebtmKWzUH433cf5Qv"}).Return(nil, assert.AnError)
			},
		}, {
			name: "failed when get recomendation outbond",
			args: args{
				userID:  1,
				limit:   10,
				trackID: "trackID",
			...
			wantErr: true,
			mockFn: func(args args) {
				mockSpotifyOutbond.EXPECT().GetRecommendation(gomock.Any(), 10, "trackID").Return(nil, assert.AnError)
